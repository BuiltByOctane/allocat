...
2. **Hydration + prefetch** — on mount, `SyncProvider` (`lib/providers/SyncProvider.tsx`) calls `hydrateAllTables()` (`lib/db/hydrate.ts`), which pulls every table for the current user into IDB. Pulls are **deltas**: each table stores a `watermark` in `sync_meta` (the newest server `updated_at` it has seen) and asks only for `updated_at >= watermark`. Table behaviour is declared once in `TABLE_SPECS` — owner column, first-pull truncation, whether deletions can be reconciled — so adding a table means adding a spec, not another hand-written query. Rules that matter:
   - **Server timestamps only.** A client clock that runs fast would skip rows. `gte` (not `gt`) plus idempotent `bulkPut` makes a microsecond tie harmless.
   - **Deletions come from an id-only sweep.** A delta can never mention a row that no longer exists, so full-payload tables also fetch `select("id")` and feed the existing `selectRowsToDelete`. Truncated tables (`sms_transactions`, `activity_logs`, `asset_value_history`, `net_worth_snapshots`, `feedback`) are NOT swept — a missing id there just means "older than the cap".
   - **Truncation applies to the first pull only** (there is no watermark yet); later deltas need no cap.
   - **Degrades safely.** Without an `updated_at` column no watermark is ever stored, so a database that predates `20260912000000_delta_sync.sql` keeps doing exactly the full pulls it did before. Clearing a watermark is the recovery path for any reconcile bug.
   - One failing table no longer aborts the rest; its watermark stays put and the next reconcile re-asks for the same window.

   If `sync_meta.__userId__` differs from the active user, IDB is wiped first (multi-account device safety). If `sync_meta.__userId__` differs from the active user, IDB is wiped first (multi-account device safety). After hydration, `prefetchAllQueries()` (`lib/db/prefetch.ts`) warms the React Query cache from IDB so first navigation has no skeletons. Use `qc.fetchQuery` (not `prefetchQuery`) when adding new prefetched keys — staleTime semantics would otherwise serve stale entries across reloads.
...
**Server-side auth is request-scoped.** `createClient()` and `getAuthedUser()` (`lib/supabase/server.ts`) are wrapped in React `cache()`, so one request verifies the user once no matter how deep the action chain goes (an SMS ingest reaches `quickLogSpend` → `addAssetEntry`/`makePayment` → notify). Use `getAuthedUser()` in new actions; a bare `supabase.auth.getUser()` reintroduces a network hop per call. `cache()` never spans requests, so two users can never share a result.
